...
if __name__ == '__main__':
    try:
        if run_in_slicer:
            ...
            # changed out path
            process_gcode(
                file_path, file_path, INFILL_TYPE, MAX_FLOW, MIN_FLOW, GRADIENT_THICKNESS, GRADIENT_DISCRETIZATION, D_F, THIN_INNER_CORE
            )
            
        else:
            start = time.time()
            process_gcode(
                INPUT_FILE_NAME, OUTPUT_FILE_NAME, INFILL_TYPE, MAX_FLOW, MIN_FLOW, GRADIENT_THICKNESS, GRADIENT_DISCRETIZATION, D_F, THIN_INNER_CORE
            )
